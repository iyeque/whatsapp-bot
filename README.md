...
- 🌐 **Real-Time Web Search**: Equipped with a custom search utility to fetch current events and bypass training data cutoffs.
...
- LM Studio / Gemini / Groq API Keys
...
- `!resume` or `engage autopilot`: Instantly re-engage the bot if it's in back-off mode.
...
3. **Reasoning**: AI decides if it needs a **Web Search** `[SEARCH:]` or a **Reaction** `[REACT:]`.
...
## 🔮 Future Roadmap (Agentic Horizon)

- [ ] **Voice Loop**: Integrate Whisper (STT) and ElevenLabs (TTS) for voice note conversations.
- [ ] **Vision**: Multi-modal support for understanding sent images.
- [ ] **Function Calling**: Expanded tools for Calendar, Email, and Weather integration.
- [ ] **Real-Time Calls**: SIP/WebRTC integration for actual phone call interactions.
