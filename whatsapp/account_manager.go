...
func NewAccountManager(dbPath string, logger waLog.Logger) (*AccountManager, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create shared vector store: %v", err)
	}

	sqlDB, err := sql.Open("sqlite", "file:bot_data.db?_pragma=foreign_keys(1)&_pragma=journal_mode(WAL)&_pragma=synchronous(NORMAL)&_pragma=cache_size(-64000)&cache=shared&mode=rwc")
	if err != nil {
		return nil, fmt.Errorf("failed to open shared bot database: %v", err)
	}
...
func (am *AccountManager) CreateNewBot() (*Bot, error) {
	...
	bot, err := NewBot(client, am.container, am, botID)
	if err != nil {
		am.mutex.Unlock()
...
func (am *AccountManager) LoadBots() error {
	...
	for _, device := range devices {
		...
		if err != nil {
			am.mutex.Unlock()
			am.logger.Errorf("Failed to create bot: %v", err)
			continue
		}
		am.bots[botID] = bot
		am.mutex.Unlock()
		...
		go func(b *Bot) {
			...
			for i := 0; i < maxRetries; i++ {
				if err := b.Connect(); err != nil {
					am.logger.Errorf("Failed to connect bot (attempt %d/%d): %v", i+1, maxRetries, err)
					if i < maxRetries-1 {
